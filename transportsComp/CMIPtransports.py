...
def meridional(cmip, model, EXP, ENS, gtype, ctype, grid, outfile, runfile):
    if not os.path.isfile(outfile):
        ...
        if grid == 'Atlantic':
            ...
            basinfile = ci.cmipsave + 'CMIP' + cmip + '/basins/' + model + '_basins.nc'
            ncid = Dataset(basinfile, 'r')
            bmask = np.squeeze(ncid.variables['Atlantic'][:, :]) + np.squeeze(
                ncid.variables['IndoPacific'][:, :],
            )
            ncid.close()
        ...
        meshmask = (
            ci.cmipsave + 'CMIP' + cmip + '/mesh_masks/' + model + '_mesh_mask.nc'
        )
            ...
            if nm >= nt:
                ...
                for mm in range(nm, nt):  # mm is month in the file
                    ...
                    if ctype[:4] == 'Tbar':
                        # Velocity:
                        vo = (
                            cr.Oread3Ddata(mdict, model, Vfiles[ff], 'vo', time=mm)
                            * vmask
                        )
                        ...
                        # Temperature:
                        thetao = (
                            cr.Oread3Ddata(mdict, model, Tfiles[ff], 'thetao', time=mm)
                            * tmask
                        )
                        ...
                        if np.isnan(ns):
                            ...
                            so = (
                                cr.Oread3Ddata(mdict, model, Sfiles[fs], 'so', time=ns)
                                * tmask
                            )
                    ...
                    if (len(Dfiles) != 0) & (model != 'CSIRO-Mk3-6-0'):
                        if (model == 'GFDL-CM4') | (
                            (model == 'ACCESS-CM2') & (EXP == 'historical')
                        ):
                            # volume not thickness is saved
                            fd, nd = cr.timeFile(model, cal, units2, tt, Dfiles)
                            dzt = (
                                cr.Oread3Ddata(mdict, model, Dfiles[fd], 'volcello', nd)
                                * tmask
                            )
                        ...
                        dzt = (
                            cr.Oread3Ddata(mdict, model, VOLfiles[fd], 'volcello', nd)
                            * tmask
                        )
                        dzt = dzt / np.tile(dxt * dyt, (np.size(dzt, axis=0), 1, 1))
                    else:
                    ...
                    dzv = (
                        cr.moveData(mdict, meshmask, 'T', 'V', dzt, computation='min')
                        * vmask
                    )
                    thetaov = cr.moveData(
                        mdict,
                    ...
                    Ttrans = (
                        np.sum(np.sum(thetaov * vo * varea, axis=2), axis=0)
                        * rho
                        * cp
                        / 1e15
                    )
                    Strans = (
                        -1
                        * np.sum(np.sum(sov * vo * varea, axis=2), axis=0)
                        / Sref
                        / 1e6
                    )
                    ...
                    # Compute MOC-topdown:
                    moc_topdown = (
                        np.cumsum(np.sum(vo * varea, axis=2), axis=0)
                        / 1e6
                        / (vareax > 0)
                    )
                    # Compute MOC-bottomup:
                    moc_bottomup = (
                        -(np.cumsum(np.sum(vo * varea, axis=2)[::-1, :], axis=0))[
                            ::-1, :
                        ]
                        / 1e6
                        / (vareax > 0)
                    ...
                    moc_section = (
                        np.cumsum(np.sum(vstar * varea, axis=2), axis=0)
                        / 1e6
                        / (vareax > 0)
                    )
                    # Compute MOC-depth:
                    ...
                    moc_depth = (
                        np.cumsum(np.sum(vbar * varea, axis=2), axis=0)
                        / 1e6
                        / (vareax > 0)
                    )
                    ...
                    Mov = (
                        ...
                        * np.sum(
                            np.sum(
                                np.moveaxis(np.tile(Szon * Vszon, (nx, 1, 1)), 0, 2)
                                * varea,
                                axis=2,
                            ),
                            axis=0,
                    ...
                    Hov = (
                        np.sum(
                            np.sum(
                                np.moveaxis(np.tile(Tzon * Vszon, (nx, 1, 1)), 0, 2)
                                * varea,
                                axis=2,
                            ),
                            axis=0,
                    ...
                    Maz = (
                        -1
                        * np.sum(np.sum(vp * sp * varea, axis=2), axis=0)
                        / Sref
                        / 1e6
                    )
                    Haz = (
                        np.sum(np.sum(vp * tp * varea, axis=2), axis=0)
                        * rho
                        * cp
                        / 1e15
                    )
