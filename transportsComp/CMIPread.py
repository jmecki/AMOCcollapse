    ...
    while (ff < nf) & (found == False):
        ...
        if (dd2 >= ttf[0]) & (dd2 <= ttf[-1]):
            ...
            while (nn < len(ttf)) & (found == False):
                if (
                    cftime.num2date(ttf[nn], units, cal).year
                    == cftime.num2date(tt, units2, cal).year
                ) & (
                    cftime.num2date(ttf[nn], units, cal).month
                    == cftime.num2date(tt, units2, cal).month
                ):
                    found = True
                else:
                    nn = nn + 1
    ...
    if cdict['extraT']:
        if (
            ...
            if cdict['reg']:
                ...
                lat = np.concatenate(
                    (lat, -90 * np.ones((1, np.size(lat, axis=1)), 'float')),
                    0,
                )
...
def Oread2Ddata(cdict, model, infile, var, time=None, lev=None, mask=False):
    ...
    # Flip Up-Down:
    if (lev != None) & (
        (model == 'CFSv2-2011') | (model == 'FGOALS-gl') | (model == 'HadGEM2-AO')
    ):
